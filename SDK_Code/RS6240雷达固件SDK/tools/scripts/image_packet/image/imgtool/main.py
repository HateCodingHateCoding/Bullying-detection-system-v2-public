...
from imgtool.version import decode_version, decode_r_version, decode_m_version
...
@click.argument('imgfile')
@click.option('--level', default='4', type=click.Choice(['0', '1', '2', '3', '4']), required=False,
              help='log level')
...
@click.command(help='Print header, TLV area and trailer information '
                    'of a signed image')
def dumpinfo(imgfile, level, outfile, silent):
    dump_imginfo(imgfile, level, outfile, silent)
...
def validate_r_version(ctx, param, value):
    if value != None:
        try:
            decode_r_version(value)
            return value
        except ValueError as e:
            raise click.BadParameter("{}".format(e))

def validate_m_version(ctx, param, value):
    if value != None:
        try:
            decode_m_version(value)
            return value
        except ValueError as e:
            raise click.BadParameter("{}".format(e))
...
@click.option('--cpu-id', type=BasedIntParamType(), required=False,
              help='cpu id.')
@click.option('--img-id', type=BasedIntParamType(), required=False,
              help='image id.')
@click.option('--start-run', default=False, is_flag=True,
              help='set pc to enter point, start run.')
@click.option('--subimg-order-auto', default=True, is_flag=True,
              help='subimg automatically arranges in merge img.')
@click.option('--merged-img', default=False, is_flag=True,
              help='this is merged image.')
@click.option('--name', required=False, default=None, help='image name.')
@click.option('--move-addr', required=False, nargs=3, default=[],
              multiple=True, metavar='[src] [dst][size]',
              help='move "size" length data from "src" address to "det" '
              'address.')
@click.option('--enter-point', type=BasedIntParamType(), required=False,
              help='Enter point address for image when it should run.')
@click.option('--remap-addr', required=False, nargs=4, default=[],
              multiple=True, metavar='[type][src] [dst][size]',
              help='virtual address remapping, '
              'remap type, physics address, virtual address, remap size.')
@click.option('--subimg-load', required=False, nargs=1, default=[],
              multiple=True, metavar='[offset]',
              help='current image infomation, image size, current offset in merge img.')
...
@click.option('-S', '--slot-size', type=BasedIntParamType(), required=False,
...
@click.option('-rv', '--rversion', callback=validate_r_version,  required=False)
@click.option('-mv', '--mversion', callback=validate_m_version,  required=False)
...
@click.option('--vector-to-sign', type=click.Choice(['payload', 'digest']),
              ...
              'signing')
@click.option('--format', type=click.Choice(['mcuboot', 'brom']),
              default='mcuboot', help="Select mcuboot or brom image format")
@click.option('--bootloader-section-size', type=BasedIntParamType(), required=False,
              help='Primary section offset.')
@click.option('--data-storage-size', type=BasedIntParamType(), required=False,
              help='Data storage size.')   
@click.option('--sram-base-addr', type=BasedIntParamType(), required=False,
              help='SRAM base address.') 
@click.option('--sram-max-size', type=BasedIntParamType(), required=False,
              help='SRAM max size.') 
@click.option('--flash-max-size', type=BasedIntParamType(), required=False,
              help='flash max size.')          
@click.command(help='''Create a signed or unsigned image\n
               INFILE and OUTFILE are parsed as Intel HEX if the params have
               .hex extension, otherwise binary format is used''')
def sign(key, public_key_format, align, version, rversion, mversion, pad_sig, header_size,
         pad_header, slot_size, pad, confirm, max_sectors, overwrite_only,
         endian, encrypt_keylen, encrypt, infile, outfile, format,
         cpu_id, img_id, merged_img, subimg_order_auto, start_run, move_addr, 
         enter_point, subimg_load, remap_addr, name, dependencies,
         hex_addr, erased_val, save_enctlv, security_counter,
         boot_record, custom_tlv, max_align, clear, fix_sig,
         fix_sig_pubkey, sig_out, vector_to_sign,
         bootloader_section_size,
         data_storage_size,
         sram_base_addr,
         sram_max_size,
         flash_max_size):
    ...
    if confirm:
        ...
        pad = True
    img = image.Image(version=decode_version(version), rversion=decode_r_version(rversion), mversion=decode_m_version(mversion),
                      cpu_id=cpu_id, img_id=img_id, merged_img=merged_img, subimg_order_auto=subimg_order_auto, 
                      start_run=start_run, move_addr=move_addr, enter_point=enter_point, 
                      format=format, remap_addr=remap_addr, name=name, header_size=header_size,
                      pad_header=pad_header, pad=pad, confirm=confirm,
                      align=int(align), slot_size=slot_size,
                      max_sectors=max_sectors, overwrite_only=overwrite_only,
                      endian=endian, erased_val=erased_val,
                      save_enctlv=save_enctlv,
                      security_counter=security_counter, max_align=max_align,
                      bootloader_section_size=bootloader_section_size,
                      data_storage_size=data_storage_size,
                      sram_base_addr=sram_base_addr,
                      sram_max_size=sram_max_size,
    ...
    if raw_signature is not None:
        if fix_sig_pubkey is None:
            raise click.UsageError('public key of the fixed signature '
                'is not specified')
    ...
    img.create(key, public_key_format, enckey, dependencies, boot_record,
               name, move_addr, enter_point, remap_addr, subimg_load,
               custom_tlvs, int(encrypt_keylen), clear,
               baked_signature, pub_key, vector_to_sign)
