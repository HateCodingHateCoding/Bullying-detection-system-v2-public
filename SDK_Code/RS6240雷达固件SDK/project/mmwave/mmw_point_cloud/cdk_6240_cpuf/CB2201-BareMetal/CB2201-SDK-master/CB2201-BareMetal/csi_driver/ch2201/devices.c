...
const sg_acmp_config[CONFIG_ACMP_NUM] = {
    {CSKY_CMP_CTL_BASE, CMP_IRQn},
};

/**
  \brief       get acmp instance count.
  \return      adc instance count
*/
int32_t target_get_acmp_count(void)
{
    return CONFIG_ACMP_NUM;
}
...
{
    if (idx >= target_get_acmp_count()) {
        return -1;
    }

    *base = sg_acmp_config[idx].base;
    *irq = sg_acmp_config[idx].irq;
    return idx;
}
