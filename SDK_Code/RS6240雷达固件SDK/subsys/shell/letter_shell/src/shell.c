...
void shellPrint(Shell *shell, const char *fmt, ...)
{
    ...
    va_start(va, fmt);
    char buffer[1];
    _vsnprintf(shell->outChar, buffer, (size_t)-1, fmt, va);
    va_end(va);

}

void shellHexPrint(Shell *shell, const void * buff, int size)
{
    int idx = 0;
    ...
    shellPrint(shell, "\n");
    for (; idx < size; idx++) {
        shellPrint(shell, "%02X ", outBuff[idx]);
        if ((idx % 16) == 15) {
            shellPrint(shell, "\n");
        }
    }
...
void shellListItem(Shell *shell, ShellCommand *item)
{
    ...
    shellWriteString(shell, "    ");
    spaceLength = 16 - shellWriteString(shell, shellGetCommandName(item));
...
unsigned int shellRunCommand(Shell *shell, ShellCommand *command)
{
    ...
    if (command->attr.attrs.type == SHELL_TYPE_CMD_MAIN)
    {
        ...
        int (*func)(Shell *, int, char **) = command->data.cmd.function;
        returnValue = func(shell, shell->parser.paramCount, shell->parser.param);
 ...
 * @param argv 参数
 */
void shellHelp(Shell *shell, int argc, char *argv[])
{
//    Shell *shell = shellGetCurrent();
    SHELL_ASSERT(shell, return);
    if (argc == 1)
